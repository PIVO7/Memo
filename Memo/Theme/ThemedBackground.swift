...
/// zeedieren in de Oceaan, en voor Klassiek een paar kaartjes op 5% inkt
...
/// Een handvol memoblokjes in bijna onzichtbare inkt: kaartstapeltjes en
/// een enkel sterretje langs de randen. Speelser dan effen, zonder de rust
/// te verliezen.
private struct ClassicFieldView: View {
    /// Posities als fractie van het scherm; `star` wisselt kaartstapeltje
    /// en sterretje af.
    private static let blocks: [(x: CGFloat, y: CGFloat, scale: CGFloat, star: Bool)] = [
        ...
        GeometryReader { geo in
            ZStack {
                ForEach(Array(Self.blocks.enumerated()), id: \.offset) { _, block in
                    Image(systemName: block.star ? "star.fill" : "rectangle.on.rectangle.angled")
